...
#### Сервер:
| Название | База данных | Фреймворк | Описание |
| - | - | - | - |
| `microservices/auth` | `PostgreSQL` или `MongoDB` | `Express` | Модуль для аутентификации и авторизации |
| `microservices/content` | `MongoDB` | `Nest`, `FirstAPI` (с генерацией контроллеров на основе `OpenAPI`) | Отвечает за работы с страницами и контентом |
| `microservises/tasks` | `PostgreSQL` | `Nest` с `NestJS tRPC` | Управление задачами. Есть одна загвоздка, задачи по сути должны наследоваться от `blocks` из `microservises/content`, надо продумать как |
| `microservises/messages` | `MongoDB` | `Fastify` | Хранит в себе комментарии от заметок и задач, использовать `WebSocket` для обновления комментариев. Сам модуль может использовать не только для комментариев, но и для написания чата |
